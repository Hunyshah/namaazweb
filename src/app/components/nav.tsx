...
// const nownewTime:any = convertTimeStringToDate(now)
  ...
  // bg-slate-900
  return (
    <div style={{backgroundColor:color}}  className={` flex flex-row justify-between items-center   text-white border-b-2 pb-4  ${roboto.className}`}>
       <div className='flex justify-evenly items-center  w-[22%]'>
        ...
        <span className='block bg-slate-50 h-24 w-0.5'></span>
            <div className='text-[300%] font-mono  min-w-[320px] '>{currentTime && <p  className={` ${orbriton.className} `}>{currentTime}</p>}</div>
